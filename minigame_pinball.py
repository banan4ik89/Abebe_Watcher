...
def start_pinball(root):
    ...
    tk.Label(win, text="ПИНБОЛ: Аластор vs Вокс",
             font=("Arial", 14, "bold")).pack(pady=10)

    tk.Label(win, text="Выбери сторону").pack(pady=5)

    def choose(player):
        win.destroy()
        game_window(root, player)

    tk.Button(win, text="Играть за Аластора",
              width=20, command=lambda: choose("alastor")).pack(pady=4)

    tk.Button(win, text="Играть за Вокса",
...
def game_window(root, player):
    ...
    def update_score():
        score_label.config(
            text=f"Раунд {round_num}/5 | Ты: {player_score} | Противник: {enemy_score}"
        )
    ...
    def countdown():
        nonlocal time_left
        time_left -= 0.1
        timer_label.config(text=f"Время: {time_left:.1f}")
        if time_left > 0:
