...
from utils import block_esc, safe_destroy, get_exe_dir
from config import DATA_DIR, DEATH_IMAGE, HINTS
        ...
        fake_deleting(root, lambda:
            fake_permission(
                ...
                lambda: fake_permission(
                    root,
                    "Доступ к микрофону",
                    "Приложение запрашивает доступ к микрофону",
...
def fake_permission(root, title, text, next_step):
    ...
    tk.Button(win, text="Да", width=10, command=approve).pack(side="left", padx=40, pady=20)
    tk.Button(win, text="Да", width=10, command=approve).pack(side="right", padx=40, pady=20)
...
def show_hack_screen(root):
    ...
    def spawn_error():
        ...
        tk.Label(win, text="Ошибка системы", font=("Arial", 10, "bold"), bg="lightgray").pack(anchor="w", padx=8)
